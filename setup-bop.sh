...
# - Pins our INPUT jump(s) for ports 443 and 80 to the TOP
...
ensure_chain() {
  CH="$1"
  iptables -S "$CH" >/dev/null 2>&1 || run iptables -N "$CH" || true
}
...
ensure_jump_top() {
  PORT="$1"; CH="$2"
  ensure_chain "$CH"
  # Remove any existing identical jump (ignore failure)
  iptables -C INPUT -p tcp --dport "$PORT" -j "$CH" 2>/dev/null && run iptables -D INPUT -p tcp --dport "$PORT" -j "$CH" || true
  # Insert at position 1
  run iptables -I INPUT 1 -p tcp --dport "$PORT" -j "$CH"
}
...
download_bop() {
  ...
  # Return path ONLY on stdout
  printf '%s
' "$TMP"
}

# Map chain name for a given port (443->BOP_HTTPS, 80->BOP_HTTP, else BOP_PORTXXXX)
chain_for_port() {
  P="$1"
  case "$P" in
    443) echo "BOP_HTTPS";;
...
EOF
  ...
  # Ensure our jumps are at the TOP and create per-port allowlists
  for P in $PORTS; do
    CH=`chain_for_port "$P"`
    ensure_jump_top "$P" "$CH"
    # First sync run to populate chain and append final DROP
